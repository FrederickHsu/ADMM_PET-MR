...
//printf("cy2= %d cz2= %d ",cy2,cz2);
        ...
        /*
	 /////   count  one  file  of  pTable/pIncre/pSparse  
        if (iz==0 && jy==0 && kcz==0 && kcy==0)
        { 
		printf("pSparse[%d] = %f \n",ktotal,pSparse[ktotal]);
        }
         */
        }
       }
   ...
   /*int a,limit;
   limit=(nimgx*nimgy*nimgz)*(nccol*ncrow);
   for (a=0;a<limit;a++)
   {
        if (x_plane==-1 && i==0 && j==0)
        {
            if (pSparse!=0)
	    {
   	        printf("system_matrix_%d_%s[%d] = %1.0f \n",x_plane,kernel_name[i*4+j],a,pSparse[a]);
            }
        }
   }*/
   fclose(inTAB);
...
int main()
{
 ...
 FILE *iHist;
 iHist=fopen("../histogram/histogram_activity_f.dat","rb");
...
/////////////////////////////////start em////////////////////////////////////////////
 ...
 for(ite=0;ite<1;ite++)
 {
  ...
  for(x_plane=0;x_plane<1;x_plane++)
  {	
        x_plane=-x_plane*2;
	forward(pFwd,pBwd);
        x_plane=-x_plane/2;
  ...
  if((ite+1)%1==0)
  {
    sprintf(out_name,"Recons/DHAPET_ite%d_activity_f.DAT",ite+1);
    FILE *fp2;
